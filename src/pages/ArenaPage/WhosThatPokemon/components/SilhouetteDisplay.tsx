...
export const SilhouetteDisplay: React.FC<SilhouetteDisplayProps> = ({
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between bg-gradient-to-b from-slate-50 via-slate-100/60 to-slate-100/95 border border-slate-200/90 rounded-3xl p-5 sm:p-6 relative overflow-hidden select-none">
      {/* Studio Dais Backdrop */}
      <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
        <div className="w-64 h-64 sm:w-80 sm:h-80 rounded-full border border-slate-200/60 bg-white/50" />
        <div className="absolute w-44 h-44 sm:w-56 sm:h-56 rounded-full border border-slate-200/40 bg-white/40" />
      </div>
            ...
            <img
              ...
              className={`max-h-[260px] sm:max-h-[300px] w-auto object-contain transition-all duration-500 select-none ${
                ...
                isRevealed
                  ...
                  : 'brightness-0 opacity-90 scale-95 contrast-200'
              }`}
            />
        ...
        <AnimatePresence mode="wait">
          {isRevealed && pokemon ? (
            <motion.div
              ...
              className="w-full flex flex-wrap items-center justify-between gap-3 bg-white border border-slate-200/90 px-4 py-2.5 rounded-2xl shadow-2xs"
            >
              <div className="flex items-center gap-2.5">
                <span className="font-mono text-xs font-bold text-amber-600">
                  #{String(pokemon.id).padStart(4, '0')}
                </span>
                <span className="text-base font-black text-slate-900 font-display tracking-tight">
                  {pokemon.displayName}
                </span>
              ...
              <div className="flex items-center gap-2">
                {isNewlyUnlocked ? (
                  ...
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-slate-600 text-xs font-medium">
